...
const apiKey = process.env.REACT_APP_GEMINI_API_KEY || ""; // Injected by environment
...
const App = () => {
  const [appUser, setAppUser] = useState({ id: 'demo-user', name: 'Usuario Demo', role: 'admin' }); // Usuario mock
  ...
  const [newReportMeta, setNewReportMeta] = useState({
    patientName: '', patientDni: '', dob: '', socialSecurity: '', diagnosis: '', type: 'EVOLUCION'
  });

  // Cargar reportes desde localStorage
  useEffect(() => {
    const savedReports = localStorage.getItem('clinica-reports');
    if (savedReports) {
  ...
  const saveReportsToStorage = (newReports) => {
    localStorage.setItem('clinica-reports', JSON.stringify(newReports));
    setReports(newReports);
  };

  const handleCreateReport = () => {
    if (!newReportMeta.patientName) return alert('Nombre requerido');
    const newDoc = {
      ...newReportMeta,
      content: {},
      createdAt: Date.now(),
    ...
    const updatedReports = [...reports, newDoc];
    saveReportsToStorage(updatedReports);
    setCurrentReport(newDoc);
    setView('edit');
  };

  const handleSaveReport = () => {
    if(!currentReport) return;
    const updatedReports = reports.map(r => r.id === currentReport.id ? { ...currentReport, updatedAt: Date.now(), lastEditor: appUser.name } : r);
    saveReportsToStorage(updatedReports);
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-teal-600 text-white p-2 rounded-lg"><Activity className="text-teal-600 w-8 h-8" /></div>
            <div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Buscar por paciente, DNI o tipo..." className="w-full pl-10 pr-4 py-2 border rounded-md focus:ring-2 focus:ring-teal-500 outline-none" value={filterText} onChange={e => setFilterText(e.target.value)} />
              </div>
                        ...
                        <div key={report.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div className="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">Por: {report.creatorName}</p>
                                </div>
                            </div>
        ...
        {view === 'create' && (
           <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
             ...
             <div className="p-6 space-y-4">
                <Input label="Nombre del Paciente" value={newReportMeta.patientName} onChange={e => setNewReportMeta({...newReportMeta, patientName: e.target.value})} />
                <div className="grid grid-cols-2 gap-4">
                    <Input label="DNI" value={newReportMeta.patientDni} onChange={e => setNewReportMeta({...newReportMeta, patientDni: e.target.value})} />
                    <Input label="Fecha Nacimiento" type="date" value={newReportMeta.dob} onChange={e => setNewReportMeta({...newReportMeta, dob: e.target.value})} />
                </div>
                <Input label="Obra Social" value={newReportMeta.socialSecurity} onChange={e => setNewReportMeta({...newReportMeta, socialSecurity: e.target.value})} />
                <Input label="Diagnóstico Base" value={newReportMeta.diagnosis} onChange={e => setNewReportMeta({...newReportMeta, diagnosis: e.target.value})} />
                <Select label="Tipo de Formulario" value={newReportMeta.type} onChange={e => setNewReportMeta({...newReportMeta, type: e.target.value})} options={Object.entries(FORM_TYPES).map(([k,v]) => ({ value: k, label: v }))} />
